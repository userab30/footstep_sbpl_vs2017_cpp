...
namespace footstep_planner
{
	...
	bool
		FootstepPlannerEnvironment::collision_check(double x, double y, double theta, double height,
			double width, int accuracy,
			gridmap_2d::GridMap2D & distance_map)
	{
		...
		FootstepPlannerEnvironment::updateHeuristicValues()
	{
		...
		if (ivHeuristicConstPtr->getHeuristicType() == Heuristic::PATH_COST)
		{
			...
			if (!success)
			{
				PRINT_ERROR("Failed to calculate path cost heuristic.");
				exit(1);
			}
		}
			...
			std::vector<int> *CostV)
	{
		...
		//PRINT_INFO("GetSuccsTo %d -> %d: %f", SourceStateID, goalStateId, euclidean_distance(current->getX(), current->getY(), ivStateId2State[goalStateId]->getX(), ivStateId2State[goalStateId]->getY()));
		...
		if (randomGoal->getLeg() != current->getLeg() && reachable(*current, *randomGoal)) {
			...
			CostV->push_back(cost);
			//       		PRINT_INFO("%d %d", goalStateId, cost);
	...
	void FootstepPlannerEnvironment::GetRandomNeighs(const PlanningState* currentState, std::vector<int>* NeighIDV, std::vector<int>* CLowV, int nNumofNeighs, int nDist_c, bool bSuccs)
	{
		...
		for (int i = 0; i < nNumofNeighs && nAttempts < 5 * nNumofNeighs; ++i, ++nAttempts)
		{
			...
			//    			i--;
			//    			PRINT_INFO("Outside of map: %d %d", newX, newY);
			//    			continue;
			//    		}
		...
		if (NeighIDV->size() == 0) {
			PRINT_WARN("Could not create any random neighbor nodes (%d attempts) from id %d (%d %d)",
				nAttempts, currentState->getId(), X, Y);
		}
		else

			PRINT_DEBUG("Created %zu random neighbors (%d attempts) from id %d "
	...
	{
		//  PRINT_ERROR("FootstepPlanerEnvironment::InitializeEnv: Hit unimplemented "
		//            "function. Check this!");
		return true;
	}
		...
		FootstepPlannerEnvironment::PrintEnv_Config(FILE *fOut)
	{
		...
		PRINT_ERROR("FootstepPlanerEnvironment::PrintEnv_Config: Hit "
		...
		FootstepPlannerEnvironment::SetAllActionsandAllOutcomes(CMDPSTATE *state)
	{
		...
		PRINT_ERROR("FootstepPlannerEnvironment::SetAllActionsandAllOutcomes: Hit"
			" unimplemented function. Check this!");
	}
		...
		FootstepPlannerEnvironment::SetAllPreds(CMDPSTATE *state)
	{
		...
		PRINT_ERROR("FootstepPlannerEnvironment::SetAllPreds: Hit unimplemented "
			"function. Check this!");
	}
